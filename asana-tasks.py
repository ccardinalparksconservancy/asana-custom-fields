...
def getEvents(projectName, projectId, sync):
    print('Starting {proj} thread with sync token {sync}.'.format(proj=projectName, sync=sync))
    log(projectName, 'Starting {proj} thread with sync token {sync}.'.format(proj=projectName, sync=sync))
    ...
    try:
        ...
        print('Error on getEvents() with project {proj} and sync {sync}: {err}.'.format(proj=projectName, sync=sync, err=str(e)))
        log(projectName, 'Error on getEvents() with project {proj} and sync {sync}: {err}.'.format(proj=projectName, sync=sync, err=str(e)))
